...
export default function TimetableStructurePage() {
  ...
  useEffect(() => {
    if (currentStructure && currentStructure.id) {
      setStructure({
        schoolId: currentStructure.schoolId,
        periodsPerDay: currentStructure.periodsPerDay,
        workingDays: currentStructure.workingDays,
        timeSlots: currentStructure.timeSlots,
        isActive: currentStructure.isActive,
      });
    }
  }, [currentStructure]);
  ...
  const addTimeSlot = () => {
    const newPeriod = structure.timeSlots.length + 1;
    const lastSlot = structure.timeSlots[structure.timeSlots.length - 1];
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <Badge variant="outline">{structure.periodsPerDay}</Badge>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center">
                ...
                <Badge variant="default">{structure.periodsPerDay * structure.workingDays.length}</Badge>
              </div>
              {structure.timeSlots.length > 0 && (
                <>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">School Starts</span>
                    <span className="font-medium">{formatTime12Hour(structure.timeSlots[0]?.startTime)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">School Ends</span>
                    <span className="font-medium">{formatTime12Hour(structure.timeSlots[structure.timeSlots.length - 1]?.endTime)}</span>
                  </div>
                </>
              )}
            </div>
                    ...
                    {structure.timeSlots.map((slot, index) => (
                      <tr key={index} className={slot.isBreak ? 'bg-orange-50' : 'bg-gray-50'}>
                        <td className="border border-gray-300 p-2 font-medium">
                          {slot.isBreak ? 'Break' : slot.period}
                        </td>
              ...
              <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <span className="font-medium ml-1">{structure.periodsPerDay}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className="font-medium ml-1">{structure.periodsPerDay * structure.workingDays.length}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Duration:</span>
                    <span className="font-medium ml-1">
                      {structure.timeSlots.length > 0 && 
                        `${formatTime12Hour(structure.timeSlots[0]?.startTime)} - ${formatTime12Hour(structure.timeSlots[structure.timeSlots.length - 1]?.endTime)}`
                      }
                    </span>
                  </div>
                </div>
